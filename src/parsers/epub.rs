...
impl PreviewParser for EpubParser {
    ...
    fn parse(&self, path: &Path) -> Result<ParsedContent, ParseError> {
        ...
        // 3. Locate spine/manifest items to read first chapter/XHTML file
        let mut book_content = format!("Title: {}\nAuthor: {}\n\n", title, author);

        // Extract text from the first available content document
        let mut text_extracted = false;
        for item_path in spine_items {
            // Resolve relative path to OPF
...
fn strip_html_tags(html: &str) -> String {
    ...
    let mut skip_content = false; // To skip <style> or <script> tags
    ...
    while i < len {
        if bytes[i] == b'<' {
            ...
            // Insert linebreaks for block level elements
            if (name_lower == "p"
                || name_lower == "/p"
                || name_lower == "br"
                || name_lower == "br/"
    ...
    // Clean up entities and normalize whitespace
    result = result
        .replace("&nbsp;", " ")
        .replace("&lt;", "<")
        .replace("&gt;", ">")
