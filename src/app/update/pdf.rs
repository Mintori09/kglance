...
pub fn handle_scrolled(
    ...
    let count = app.state.pdf.page_count;
...
pub fn handle_page_ready(
    ...
    if index < app.state.pdf.pages.len() {
        let handle = iced::widget::image::Handle::from_rgba(width, height, data.clone());
        app.state.pdf.pages[index] = Some(crate::core::PageCacheEntry {
            data,
            width,
            height,
            handle,
    ...
    let all_loaded = app.state.pdf.pages.iter().all(|p| p.is_some());
    if all_loaded {
        app.state.pdf.loading = false;
    }
    Task::none()
...
pub fn handle_thumb_ready(
    ...
    if index < app.state.pdf.thumbnails.len() {
        let handle = iced::widget::image::Handle::from_rgba(width, height, data.clone());
        app.state.pdf.thumbnails[index] = Some(crate::core::PageCacheEntry {
            data,
            width,
            height,
            handle,
