...
use crate::features::markdown::syntax::highlight_code;
use crate::features::markdown::{Block, Inline, ListItem, TableBlock, flatten_inlines};
use crate::log_debug;
...
fn render_inlines<'a>(
    ...
    if !has_link && !has_math && !has_image {
    ...
    for (i, inline) in inlines.iter().enumerate() {
        ...
        } = inline
        {
            ...
            start = i + 1;
        } else if let Inline::Image { alt, .. } = inline {
            if start < i {
                elements.push(
                    Rich::with_spans(inlines_to_spans(&inlines[start..i]))
                        .size(Pixels(font_size))
            ...
            if let Some(bi) = block_index {
                let handle = state.cached_inline_image_handles.get(&(bi, i)).cloned();
                if let Some(h) = handle {
                    let img = image(h).height(Length::Shrink);
                    let img = if let Some((w, _h)) = state.cached_inline_image_sizes.get(&(bi, i)) {
                        if *w > 600 {
                ...
                elements.push(
                    container(text(format!("[{alt}]")).size(13))
                        .padding([2, 8])
                        .into(),
                );
            }
...
fn render_heading<'a>(
    ...
    let label = render_inlines(content, size, Some(block_index), state);
...
fn render_paragraph<'a>(
    ...
    let rich = render_inlines(content, font_size, Some(block_index), state);
...
fn render_table<'a>(
    ...
    let header_cells: Vec<Element<'a, Message>> = table
        ...
        .map(|(i, h)| {
            ...
            let cell = render_inlines(&h.content, hdr_size, None, state);
    ...
    for (_i, row_data) in table.rows.iter().enumerate() {
        let cells: Vec<Element<'a, Message>> = row_data
            ...
            .map(|(j, c)| {
                ...
                let cell = render_inlines(&c.content, cel_size, None, state);
                container(cell).padding([8, 12]).width(w).into()
            })
            .collect();
...
fn render_list<'a>(
    ...
    let item_elements = items.iter().enumerate().map(|(idx, item)| {
        ...
        let content_el = render_inlines(&item.content, font_size, Some(block_index), state);
...
fn render_block<'a>(
    ...
    match block {
        Block::Heading { level, content } => {
            render_heading(*level, content, font_size, index, state)
        }
        Block::Paragraph(content) => render_paragraph(content, font_size, index, state),
        Block::CodeBlock { lang, code, .. } => render_code_block(lang, code, font_size, is_dark),
        Block::Table(tbl) => render_table(tbl, font_size, is_dark, state),
